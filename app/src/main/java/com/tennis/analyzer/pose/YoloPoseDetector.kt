...
class YoloPoseDetector(private val context: Context) {
    ...
    companion object {
        ...
        // Поднято с 0.35. Замер на COCO128 (65 картинок без людей / 62 с людьми, FP32 и INT8
        // модели дали одинаковый результат — не артефакт квантования): при 0.35 ложные
        // срабатывания на картинках БЕЗ человека — 3/65 (4.6%), вплоть до confidence 0.75.
        // При 0.5 — 2/65 (3.1%), recall на людях падает с 86.9% до 80.3% (на общих COCO-сценах —
        // толпы, дальний план, размытие; наш домен проще: один игрок на корте при нормальном
        // освещении, так что реальная потеря recall ожидается меньше).
        //
        // Важная поправка по итогам живого теста: изначально казалось что модель "видит
        // человека" там где его нет (видео дороги в машине, полноценный фиктивный результат
        // анализа). Разбор конкретных кадров (см. историю коммитов) показал, что человек там
        // РЕАЛЬНО был — пассажир крупным планом на части кадров, просто не в превью-миниатюре
        // из галереи. Настоящая причина ложного результата была не в детекции позы, а в том
        // что кинематика (взмах руки во время разговора/жестикуляции) ошибочно принималась за
        // подачу — исправлено в VideoPoseAnalyzer.hasRacketNearby(): без обнаруженной рядом
        // ракетки YOLO контакт не засчитывается, независимо от того, что показала кинематика.
        // Порог confidence и ограничение переноса landmarks между кадрами (MAX_CARRY_MS в
        // VideoPoseAnalyzer) остаются полезной защитой сами по себе, но именно они не были
        // источником этого конкретного случая.
